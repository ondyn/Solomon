...
class FlatView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        current_owner_table = tables.CurrentOwnerTable(current_owners_qs, orderable=False)
...
class PersonView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        flat_owner_ids = list(
            models.FlatOwner.objects.filter(
                owner__persons=instance
            ).select_related("owner", "flat__building").values_list("pk", flat=True)
        )
...
class PropertyOwnerView(generic.ObjectView):
    queryset = models.PropertyOwner.objects.prefetch_related("persons", "flat_owners__flat__building")
    layout = layout.SimpleLayout(
        left_panels=[
            PropertyOwnerPanel(),
            PropertyOwnerAddressPanel(),
    ...
    def get_extra_context(self, request, instance):
        ...
        person_table = tables.PersonCompactTable(instance.persons.all(), orderable=False)
