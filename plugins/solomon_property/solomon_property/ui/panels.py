...
class BuildingObjectPanel(panels.ObjectAttributesPanel):
    ...
    cuzk_building_id = attrs.TextAttr(
        "cuzk_building_id", label=_("CUZK building object ID")
    )
    ...
    cadastral_territory_name = attrs.TextAttr(
        "cadastral_territory_name", label=_("Cadastral territory")
    )
...
class BuildingNotesPanel(panels.ObjectAttributesPanel):
    ...
    common_area_rental = attrs.TextAttr(
        "common_area_rental", label=_("Common area rental info")
    )
    note = attrs.TextAttr("note", label=_("Note"))
...
class FlatInstallationsPanel(panels.ObjectAttributesPanel):
    ...
    radiator_power_kw = attrs.TextAttr(
        "radiator_power_kw", label=_("Radiator power (kW)")
    )
...
class FlatCuzkPanel(panels.ObjectAttributesPanel):
    ...
    ownership_cert_number = attrs.TextAttr(
        "ownership_cert_number", label=_("Ownership certificate number")
    )
...
class PersonPanel(panels.ObjectAttributesPanel):
    ...
    emails = attrs.TemplatedAttr(
        "emails",
        label=_("Emails"),
        template_name="solomon_property/attrs/email_list.html",
    )
    phones = attrs.TemplatedAttr(
        "phones",
        label=_("Phones"),
        template_name="solomon_property/attrs/phone_list.html",
    )
...
class PersonAddressPanel(panels.ObjectAttributesPanel):
    title = _("Addresses")
    permanent_address = attrs.TextAttr(
...
class PropertyOwnerPanel(panels.ObjectAttributesPanel):
    ...
    current_total_share = attrs.TextAttr(
        "current_total_share", label=_("Total active share")
    )
